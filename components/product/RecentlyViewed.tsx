    ...
    <AnimatePresence>
      {items.length > 0 && (
        <motion.section 
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="bg-black py-32 border-t border-white/10 px-6 md:px-12 mono"
        >
            ...
            <div className="flex items-center justify-between mb-16">
              ...
              <div className="flex items-center gap-8">
                <div className="hidden md:flex gap-4">
                   <div className="h-[1px] w-32 bg-white/10 self-center" />
                   <p className="text-[11px] font-bold text-white/60 uppercase tracking-widest self-center">Showing_Last_{items.length}_Nodes</p>
                </div>
                <button 
                  onClick={() => clearHistory()}
                  className="flex items-center gap-2 text-xs font-black uppercase tracking-widest text-white/60 hover:text-enark-red transition-colors group"
                >
              ...
              {items.map((item, i) => (
                ...
                >
                  <Link href={`/shop?product_id=${item.id}`} className="group block">
                    <div className="aspect-[3/4] bg-white/5 border border-white/10 mb-6 overflow-hidden relative">
                      <img 
                        src={item.image} 
                        alt={item.name} 
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-xs font-bold text-white/60 mono">₹{item.price.toLocaleString()}</p>
                    </div>
                  </Link>
                </motion.div>
              ))}
