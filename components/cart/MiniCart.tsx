      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed right-0 top-0 h-full w-full max-w-lg bg-background border-l border-theme z-[120] flex flex-col"
          >
            {/* Header */}
            <div className="p-8 border-b border-theme flex items-center justify-between bg-background">
              <h2 className="text-2xl font-black tracking-tighter-x uppercase">
                Your Bag
                <span className="text-enark-red ml-2">[{items.length}]</span>
              </h2>
              <button onClick={() => setIsOpen(false)} className="p-2 hover:text-enark-red transition-colors text-foreground/60">
                <X size={24} />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto overflow-x-hidden relative">
              <AnimatePresence mode="wait">
                  ...
                  >
                    {items.length === 0 ? (
                      <div className="flex-1 flex flex-col items-center justify-center p-12 text-center text-foreground/60 uppercase mono">
                        Your bag is empty
                      </div>
                    ) : (
                      <div className="divide-y border-theme">
                        {items.map((item) => (
                          <div key={item.variantId} className="flex p-8 gap-8 group">
                            <div className="w-24 h-32 bg-surface border border-theme overflow-hidden">
                            ...
                            <div className="flex-1 flex flex-col justify-between">
                              <div>
                                ...
                                <p className="mono text-[11px] text-foreground/80 uppercase tracking-widest">SKU: {item.sku}</p>
                              </div>
                              <div className="flex items-center justify-between">
                                <div className="flex border border-theme">
                                  <button onClick={() => updateQuantity(item.variantId, item.quantity - 1)} className="p-2 hover:bg-foreground/5 text-foreground/60"><Minus size={12} /></button>
                                  <span className="w-8 flex items-center justify-center mono text-xs">{item.quantity}</span>
                                  <button onClick={() => updateQuantity(item.variantId, item.quantity + 1)} className="p-2 hover:bg-foreground/5 text-foreground/60"><Plus size={12} /></button>
                                </div>
                                <button onClick={() => removeItem(item.variantId)} className="text-foreground/60 hover:text-enark-red transition-colors"><Trash2 size={14} /></button>
                              </div>
                            </div>
                          </div>
                        ))}
            ...
            {/* Footer */}
            {items.length > 0 && (
              <div className="p-8 border-t border-theme bg-background z-10">
                <div className="flex justify-between mb-8">
                  <span className="mono text-xs font-bold text-foreground/60 tracking-widest">Total</span>
                  <span className="text-2xl font-black">₹{total.toLocaleString()}</span>
                </div>
                
                <button 
                  onClick={handleCheckoutInitiation}
                  className="w-full bg-enark-red text-white py-6 flex items-center justify-between px-8 group hover:bg-foreground hover:text-background transition-all"
                >
